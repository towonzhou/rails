# -*- encoding: UTF-8 -*-
##############################################################
# File Name: goliath.rb
# Author: zhouhuan
# mail: towonzhou@gmail.com
# Created Time: 2013年10月22日 星期二 11时20分29秒
##############################################################
...
module Constant
  ORDERS = "hm_orders_list" #订单列表
  CACHE = "hm_cache_list"   #正在处理的订单列表
...
class Daemon
  ...
  def run
    EM.add_periodic_timer(1) {
      ol = redis_conn.llen(ORDERS)
      cl = redis_conn.llen(CACHE)
      p "#{Time.now.strftime("%T")}:the count in #{ORDERS}:#{ol}"
      p "#{Time.now.strftime("%T")}:the count in #{CACHE}:#{cl}"
      count = get_orders_into_cache_list(ol) if (ol > 0 && cl == 0)

      #如果大于0开始查询
      query_cache if (count || cl > 0)
    }
  end
  ...
  def get_orders_into_cache_list ol
    debug "init get_orders_into_cache_list", nil
    count = (ol < CACHE_MAX_COUNT) ? ol : CACHE_MAX_COUNT
    count.times do
